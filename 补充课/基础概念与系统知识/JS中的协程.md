...
对一个进程来说，是其条件不足，需要等待提交才能继续执行；对一个线程来说，是其在等待一个事件，逻辑上不可执行。
...
## 并行与并发
并行和并发是多个程序（进程或线程）在系统层面同时运行的不同形式，用来形容进程或线程之间的关系。
### 并行
多个程序在不同的CPU或CPU核心上运行，彼此不抢占CPU资源，可称之为并行。是真正的**同时**运行。
如：
- 操作系统拥有两个CPU时，CPU1处理进程1，CPU2处理进程2，进程1和进程2并行。
- 一个双核CPU，运行一个进程，进程中包含两个线程，那么这两个线程各自有一个CPU核心，属于并行。
### 并发
并发是指一段时间内（程序开始运行到结束的这段时间）执行多个程序（线程算是一个进程的子程序），但在任意一个时间点上，只有一个程序在CPU上运行。那么此时，CPU通常使用时间片的方式调度程序，即CPU的运行时间会被划分成若干个时间片，轮流分配给不同的程序，一个时间片只会有一个程序执行。
如：
- 操作系统只有一个CPU，此时多个进程在运行，那么这多个进程属于并发。
- 一个单核CPU，运行一个进程，进程中包含多个线程，那么这多个线程在一个CPU核心上并发。
简单来说，并发是CPU不足以单个分配给程序时的解决方案，它解决了程序排队等待的问题，如果一个程序发生阻塞，其他程序仍然可以正常执行。
