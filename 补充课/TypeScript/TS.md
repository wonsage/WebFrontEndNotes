...
### Class
#### 可访问性修饰符
写在属性或方法的最前面（比如readonly、static等关键字的前面）
##### public
表示此项是公开成员，外部可以自由访问。
public是默认修饰符，可省略不写。
##### private
表示私有成员，只能用在当前类的内部，类的实例和子类都不能使用该成员。
子类不可以定义父类私有成员的同名成员。
并非真正严格的私有成员，ES2022引入了`#`来定义私有成员，建议使用。
##### protected
表示保护成员，只能在类的内部使用该成员，实例无法使用该成员，但是子类内部可以使用。
子类可以定义父类私有成员的同名成员。
#### static
静态类
#### abstract
抽象类表示该类不能被实例化，只能作为其他类的模板。
抽象类下的成员前面可以加`abstract`，表示此成员是抽象成员，其子类必须实现该属性或方法。
### 泛型
理解为用于存储类型的变量，使用它表示一些需要保持一致但又根据情况变化的类型。